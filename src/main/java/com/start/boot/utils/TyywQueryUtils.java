 ...
 */
@Component
public class TyywQueryUtils<T>  {

    public TyywQueryUtils(){}
    ...
    public  List<Map<String,Object>> getMap(Query query) throws Exception {
        ...
        List<Map<String,Object>> results = tyywQueryUtilsMapper.query(query);
        return results;
    }
     ...
     * @throws Exception
     */
    public  List<String> getSelected(Query query) throws Exception {
        if (query.getColumnNames()==null){
            query.build();
        }
        Assert.notNull(query.getTableName(),"表名不能为空！");
        Assert.notNull(query.getColumnNames(),"请指定要查询的实体类或要查询的列名");
        List<String> results = tyywQueryUtilsMapper.querySelect(query);
        return results;
    }
