...
/**
 * 混合评查管理控制器(评查活动+评查案件+评查人员)
 * Created by 李志恒 on 2018年4月13日.
 */
@RestController
@RequestMapping("/blend")
public class BlendController extends ArchivesSystemBaseController {
    ...
    public String addPcfa(String json) {
        ...
        try {
            ...
            Param_Pcfa data = pcService.addPcfa(param);  //添加评查方案
            result = success(data, "新增评查方案成功");
            Param_Pcsp pcsp = new Param_Pcsp();   //评查方案发送审批
            pcsp.setPchdbm(data.getPchdbm());
            pcsp.setSprdwbm(getCurrentDwbm());
            pcsp.setSprdwmc(getCurrentDwmc());
            pcsp.setSprgh(getCurrentGh());
            pcsp.setSprxm(getCurrentMC());
            pcsp.setSsrdwbm(getCurrentDwbm());
            pcsp.setSsrdwmc(getCurrentDwmc());
            pcsp.setSsrgh(getCurrentGh());
            pcsp.setSsrxm(getCurrentMC());
            boolean isSuccess = pcService.addPcfaps(pcsp);
            result = success(isSuccess, "评查方案送审成功");
            Param_Pcsp pcspParam = new Param_Pcsp();
            Param_Pchdzt pchdzt = pcService.getHdztBM(data.getPchdbm());
            pcspParam.setPcspbm(pchdzt.getPcspbm());
            pcspParam.setSpjl("同意");
            pcspParam.setSpyj("同意");
            isSuccess = pcService.uptDealFasp(pcspParam);
            result = success(isSuccess, "评查方案审批成功");
            Param_Hd paramHd = new Param_Hd();
            paramHd.setPchdbm(data.getPchdbm());
            paramHd.setQdr_dwbm(getCurrentDwbm());
            paramHd.setQdr_gh(getCurrentGh());
            paramHd.setQdr_mc(getCurrentMC());
            isSuccess = pcService.startPchd(paramHd);
            result = success(isSuccess, "启动评查活动成功");
            paramHd.setPchdbm(data.getPchdbm());
            paramHd.setJsr_dwbm(getCurrentDwbm());
            paramHd.setJsr_gh(getCurrentGh());
            paramHd.setJsr_mc(getCurrentMC());
            isSuccess = pcService.finishPchd(paramHd);
            result = success(data, "新增评查方案成功");
        } catch (Exception e) {
            super.errMsg("新增评查方案失败", json, e);
            result = failure(e.getMessage(), "新增评查方案失败!原因:"+e.getMessage());
        }
