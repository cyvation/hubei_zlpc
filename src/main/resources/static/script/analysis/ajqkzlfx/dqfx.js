...
function init_monitor_overview() {
    ...
    //案件统计类别
    $('#tjfx_dq_cbt_build_ajtjlb').combotree({
        editable: false,
        panelWidth: 160,
        lines: true,
        multiple: true,
        ...
        onLoadSuccess: function (node, data) {
            if (data != null && data.length >= 1) {
                setAllCheckbox('#tjfx_dq_cbt_build_ajtjlb', data);
            }
            index_addMousedownDiv(this, "tjfx_dq_cbt_build_ajtjlb");
        }
    });
    $('#tjfx_dq_cbt_build_ajtjlb').combotree("loadData", getStajbs());
    //查询：
    ...
    //导出Excel
    $("#btn_dq_expExcel").unbind('click');
    $("#btn_dq_expExcel").bind('click', function () {
        ajzlqkfx_dq_expExcel();
    });
    ...
    init_table_analysis_ajqkzlfx_dq(); //初始化年份统计报表
}

function init_table_analysis_ajqkzlfx_dq() {

    $('#table_analysis_ajqkzlfx_dq').treegrid({
        fitColumns: true,
        striped: true,
        nowrap: false,
        rownumbers: false,
        ...
        columns: [[
            {
                field: 'name',
                title: '<span  style=\'font-size:16px;\'>地区</span>',
                rowspan: 3,
                width: 64,
                align: 'center',
                halign: 'center'
        ...
        ], [
            {
                ...
                formatter: function (value, row, index) {
                    var r = '';
                    var count = row.name.indexOf("合计");
                    if (count <= -1) {
                        r = '<a href="#"  style="color: #145bae;text-decoration: none;"   onclick="alert_jbxx_dq_tj_window(' + row.id + ', \'优质案件\')">' + value + '</a> ';
                    } else {
                        r = value;
                    }
                    return r;
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    ...
                    var count = row.name.indexOf("合计");
                    if (count <= -1) {
                        r = '<a href="#"  style="color: #145bae;text-decoration: none;"   onclick="alert_jbxx_dq_tj_window(' + row.id + ', \'合格案件\')">' + value + '</a> ';
                    } else {
                        r = value;
                    }
                    return r;
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    ...
                    var count = row.name.indexOf("合计");
                    if (count <= -1) {
                        r = '<a href="#"  style="color: #145bae;text-decoration: none;"   onclick="alert_jbxx_dq_tj_window(' + row.id + ', \'瑕疵案件\')">' + value + '</a> ';
                    } else {
                        r = value;
                    }
                    return r;
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    ...
                    var count = row.name.indexOf("合计");
                    if (count <= -1) {
                        r = '<a href="#"  style="color: #145bae;text-decoration: none;"   onclick="alert_jbxx_dq_tj_window(' + row.id + ', \'不合格案件\')">' + value + '</a> ';
                    } else {
                        r = value;
                    }
                    return r;
        ...
        loadFilter: function (result) {
            $("#btn_dq_expExcel").removeAttr("disabled");
...
function data_monitor_overview_rd() {
    ...
    obj.ajtjlb = $('#tjfx_dq_cbt_build_ajtjlb').combotree('getValues').join(",").trim();
    ...
    $('#table_analysis_ajqkzlfx_dq').treegrid({
        url: getRootPath() + "/analysis/getAjzlqkfxByDq",
        method: 'post',
        queryParams: obj
    });
}

function ajzlqkfx_dq_expExcel(){
    var obj = new Object();
    obj.wcrqnf = $('#monitor_handler_year_combotree').combotree('getValues').join(",").trim();
    if(obj.wcrqnf == ""){
        Alert("请选择年度!");
    ...
    $.ajax({
        type: 'post',
        url: getRootPath() + '/analysis/getAjzlqkfxByDqToExcel',
        dataType: 'json',
        data: obj,
        ...
        error: function (xhr) {
            Alert('导出案件质量情况地区分析错误\n\n' + xhr.responseText);
        }
    });
}

function alert_jbxx_dq_tj_window(dwbm, pcjl) {
    var obj = new Object();
    obj.wcrqnf = $('#monitor_handler_year_combotree').combotree('getValues').join(",").trim();
    obj.dwbm = dwbm;
    obj.pcflbm = $('#cbt_win_eval_build_pcfl').combotree('getValues').join(",").trim();
    ...
    // 已评查案件datagrid
    $("#table_analysis_dq_filter").datagrid({
        border: 0,
        fit: true,
        fitColumns: true,
        singleSelect: true,
        ...
        columns: [[
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    ...
                    if (pcflbm == '009') {
                        return ' <a href="#" onclick="pcWin_analysis_dq_pcyl(' + index + ')">查看</a>   ';
                    } else {
                        return '<a href="#" onclick="pcWinPage(' + index + ',\'#table_analysis_dq_filter\',0)">查看</a>';
                    }
                }
            }
        ]]
    ...
    // 分页控件(中文)
    $('#table_analysis_dq_filter').datagrid('getPager').pagination({
        beforePageText: '第',
        afterPageText: '页   共{pages}页',
        displayMsg: '当前显示【{from} ~ {to}】条记录   共【{total}】条记录'
    });

    $("#win_analysis_dq_filter").window('setTitle', '已经评查案件');
    $("#win_analysis_dq_filter").window('open');
}

function pcWin_analysis_dq_pcyl(index) {
    ...
    var datas=$('#table_analysis_dq_filter').datagrid('getRows')[index];
