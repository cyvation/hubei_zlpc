...
function combobox_analysis_report_pcfs() {
    $("#combobox_analysis_report_pcfs").combotree({
        ...
        onLoadSuccess: function () {
            //默认选中所有方式
            var fl_arr = [];
            var data = $("#combobox_analysis_report_pcfs").combotree('tree').tree('getRoots');
            $.each(data, function (i) {
                fl_arr.push(data[i].id)
            })
            $("#combobox_analysis_report_pcfs").combotree('setValues', fl_arr);
            pcfs = "'" + $("#combobox_analysis_report_pcfs").combobox('getValues').join("','") + "'";
            //加载评查单位树
...
function tree_analysis_report_pcdw() {
    $('#tree_analysis_report_pcdw').tree({
        method: 'get',
        editable: false,
        // panelWidth: 250,
        lines: true,
        ...
        onLoadSuccess: function (node, data) {
            ...
            if (data.length > 0) {
                ...
                //找到第一个元素
                var n = $('#tree_analysis_report_pcdw').tree('find', userInfo.DWBM/*data[0].id*/);
                //调用选中事件
                $('#tree_analysis_report_pcdw').tree('select', n.target);
            }
...
//下一步按钮点击事件
    $("#btn_wind_eval_next").linkbutton({
        onClick: function () {
            add_pcfs = $("#cbt_win_eval_build_pcfl").combobox("getValue");//设置新增报告的评查方式
            close_win_add_rept();
            if (add_pcfs != '010') {
                clear_upload_win_rept();
                open_win_upload_rept();
            }
...
function load_rept_datagrid() {
    ...
    var current_select_unit = "'" + $("#tree_analysis_report_pcdw").combobox('getValues').join("','") + "'";
    var obj = {dwbm: current_select_unit, pcflbm: pcfs}
    $('#dg_pcbg_rept').datagrid({
        width: 'auto',
        striped: true,
        singleSelect: true,
        ...
        rownumbers: true,
        pagination: true,
        url: getRootPath() + '/report/getPcbg',
        queryParams: obj,
