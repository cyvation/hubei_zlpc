...
function init_special_toolbar() {
    ...
    $.ajax({
        type: 'POST',
        url: getRootPath() + '/manage/getHdztBM',
...
function eval_special_marksInit(pcxx) {
    ...
    $('#cbt_eval_bulid_sp_sxgz').combotree({
        ...
        queryParams: {
            pcflbm: pcxx.PCFLBM,
            sslb: "",
            pcmbbm: pcxx.PCMBBM  //添加模板编码参数 LZH 2018年4月2日
        },
        onLoadSuccess:function(node,data){
            if(data.length > 0){
    ...
    /*// 筛选
     $("#btn_eval_build_sp_filter").unbind( "click" );
     $("#btn_eval_build_sp_filter").bind("click", function () {
    ...
    // 折叠
    // $(".ZXpc_pcfa_but").toggle(function () {
    //     $("#div_eval_build_sp_pcz").hide(500);
    //     $(".ZXpc_pcfa_cen").animate({height: "560px",}, 500 );
    //     $(".ZXpc_pcfa_cen1").animate({height: "560px",}, 500 );
    // },function () {
    //     $("#div_eval_build_sp_pcz").show(500);
    //     $(".ZXpc_pcfa_cen").animate({height: "430px",}, 500 );
    //     $(".ZXpc_pcfa_cen1").animate({height: "430px",}, 500 );
    // });
...
function start_eval_special(){
    Confirm("确认", "是否启动评查活动？", function (r) {
        if (r) {
            ...
            $.ajax({
                type: 'POST',
                url: getRootPath()+'/manage/startPchd',
...
function finish_eval_special(){
    Confirm("确认", "是否结束评查活动？", function (r) {
        if (r) {
            ...
            $.ajax({
                type: 'POST',
                url: getRootPath()+'/manage/finishPchd',
                data: { json: JSON.stringify(obj) },
                dataType: "json",
                success: function (result) {
...
function deal_eval_special_approve(){
    ...
    //设置radio内name名字
    // $('.spyj_btn').attr('name','is_check_of_sp');
    // $("input[name='is_check_of_sp']").attr("disabled",false);
    // $("input[name='is_check_of_sp']").removeAttr("checked");
    ...
    $("#btn_sp_bg_confirm").bind("click", function () {
        ...
        $.ajax({
            url: getRootPath() + "/manage/uptDealFasp",
            data: { json: JSON.stringify(obj)},
            type: 'post',
            async: true,
            dataType: 'json',
...
function send_eval_special_approve() {
    ...
    $('#grid_win_pcfa_agfzr').datagrid({
        ...
        loadMsg: '数据加载中，请稍后...',
        rownumbers: true,
        url: getRootPath()+'/manage/getHdsp',
        idField: 'ID',
        columns: [[
    ...
    $("#btnFAConfirm").bind("click", function () {
        ...
        obj.SPRXM = row.MC;
        $.ajax({
            url: getRootPath() + "/manage/addPcfaps",
            data: { json: JSON.stringify(obj)},
            type: 'post',
            async: true,
            dataType: 'json',
...
function generate_pcfa(isOverride) {
    ...
    //$('#div_pcfa_doc').css('display','block');
    ...
    $.ajax({
        ...
        success: function(data) {
            if(data.code==200){
                ...
                $.ajax({
                    type: 'POST',
                    url: getRootPath()+'/manage/generateDoc',
                    data: { json: JSON.stringify(obj) },
                    dataType: "json",
                    success: function (result) {
...
function save_pcfa_file() {
    ...
    // 编辑保存
    var url = getRootPath() + "/manage/uploadFile?wjlj=" + EVAL_DATA.PCFALJ;
...
function load_eval_build_wenshu(){
    ...
    $.ajax({
        type: 'POST',
        url: getRootPath()+'/manage/getPcfaPath',
...
function load_table_eval_bulid_sp_filter(pcxx) {
    ...
    obj.GZDWBM = '';
    obj.SXGZBM = $('#cbt_eval_bulid_sp_sxgz').combotree('getValue');
    obj.CBDWBM =  $('#cbt_eval_bulid_sp_dwbm').combotree('getValues').join(",");
    obj.CBBMBM = $('#txt_eval_build_sp_cbbm').combotree('getValues').join(",");
    obj.AJLB = '';
    obj.BMSAH = $('#txt_eval_build_sp_bmsah').textbox('getValue');
    obj.AJMC = $('#txt_eval_build_sp_ajmc').textbox('getValue');
    obj.CBRXM = $('#txt_eval_build_sp_cbr').textbox('getValue');
    obj.AY = '';
    obj.SLRQBNG = '';
    obj.SLRQEND = '';
    obj.BJRQBNG = '';
...
function init_eval_build_special(data) {
    ...
    $("#btn_save_eval_build_sp_info").bind("click", function () {
        ...
        // 更新评查方案信息
        $.ajax({
            url: getRootPath() + "/manage/updPchd",
            data: { json: JSON.stringify(obj)},
            type: 'post',
            async: false,
            dataType: 'json',
...
function click_btn_wind_eval_next(pcxx){
    ...
    // 生成评查方案
    $.ajax({
        url: getRootPath() + "/manage/addPcfa",
        data: { json: JSON.stringify(obj)},
        type: 'post',
        async: false,
        dataType: 'json',
...
function init_eval_build_special_assign() {
    ...
    $('#table_eval_bulid_sp_assign').datagrid({
        ...
        columns:[[
            ...
            {field: 'action', title: '操作', width: 90, align: 'center',
                formatter: function (value, row, index) {
                    var r = '<a href="#" onclick="pcWinPage(' + index +',\'#table_eval_bulid_sp_assign\',1)">查看</a> ';
                    /*var r = '<a href="#" onclick="lookup_special_case(\'' + row.BMSAH + '\')">查看</a> ';*/
                    // var d = '<a href="#" onclick="assign_special_case(' + index + ',\'' + row.BMSAH + '\')">分配</a> ';
                    var d = '<a href="#" onclick="assign_special_case(' + index + ')">分配</a> ';
                    return r + d;
                }
            }
        ]],
    ...
    /*$('#table_eval_bulid_sp_assign').datagrid('getPager').pagination({
     beforePageText: '第',
     afterPageText: '页   共{pages}页',
     displayMsg: '当前显示【{from} ~ {to}】条记录   共【{total}】条记录'
     });*/
...
function assign_random_pcy() {
    ...
    /*// 评查员列表
     ...
     PCZ_LIST[i].PCYARR[j].PCZBM = PCZ_LIST[i].PCZBM;
     PCZ_LIST[i].PCYARR[j].PCZMC = PCZ_LIST[i].PCZMC;
     pcyArr.push(PCZ_LIST[i].PCYARR[j]);
     }
     }

     // 评查案件随机分配
     var length = pcyArr.length;
     for (var i = 0; i < EVAL_DATA.PCAJLB.length; i++){
     if (!EVAL_DATA.PCAJLB[i])
     continue;

     var num = getRandomNumWithoutM(0, length);
     EVAL_DATA.PCAJLB[i].PCZ_BM = pcyArr[num].PCZBM;
     EVAL_DATA.PCAJLB[i].PCZ_MC = pcyArr[num].PCZMC;
     EVAL_DATA.PCAJLB[i].PCR_GH = pcyArr[num].GH;
     EVAL_DATA.PCAJLB[i].PCR_MC = pcyArr[num].MC;
     EVAL_DATA.PCAJLB[i].PCR_DWBM = pcyArr[num].DWBM;
     EVAL_DATA.PCAJLB[i].PCR_DWMC = pcyArr[num].DWMC;
     }

     $('#table_eval_bulid_sp_assign').datagrid('loadData', EVAL_DATA.PCAJLB);
     var pcajlb = $("#table_eval_bulid_sp_assign").datagrid("getRows");
     // 分配案件
     $.ajax({
     url: getRootPath() + "/manage/assignCase",
     data: { json: JSON.stringify(pcajlb)},
     type: 'post',
     async: true,
     dataType: 'json',
     success: function (data) {
    ...
    $.ajax({
        type: 'POST',
        url: getRootPath() + '/manage/invokeSf',
        data: obj,
        async: true,
        dataType: 'json',
        success: function (result) {
...
function assign_special_case(index) {
    window.event ? window.event.cancelBubble = true : e.stopPropagation();
    // var index = $("#table_eval_bulid_sp_assign").datagrid("getRowIndex",row);
...
function alert_win_eval_build_sp_pcz(pczbm) {
    ...
    $("#btn_eval_build_sp_pcz_confirm").bind("click", function () {
       // var a= $('#table_eval_build_sp_pcz_ry').datagrid('getEditingRowIndexs');
       // if(a[0]){
       //  $('#table_eval_build_sp_pcz_ry').datagrid("endEdit",a[0]);
       //  $('#table_eval_build_sp_pcz_ry').datagrid("unselectAll");
       // }
        $('td[field="DWBM"]').click();
        // 评查组信息
        //pcz.PCHDBM = IS_EDIT ? EVAL_DATA.PCDWBM : "";
...
function getFpsfList() {
    $.ajax({
        type: 'POST',
        url: getRootPath()+'/manage/getsfList',
        success:function (data) {
