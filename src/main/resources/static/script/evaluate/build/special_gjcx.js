...
// 完成日期框初始化
function init_gjcx_datebox() {

    //完成日期
    $('#date_eval_build_gjcx_dept_begin').datebox({
        editable: false,
        value: new Date().getFullYear() + '-01-01'
    });

    $('#date_eval_build_gjcx_dept_end').datebox({
        editable: false,
...
function loadFixedDwData(){
    $.ajax({
        ...
        success: function (data) {
            ...
            //$('#value_fixed').combotree('loadData',dwbm_jsonData);
            $('#value_fixed').combotree({
                data:dwbm_jsonData,
                onShowPanel: index_onShowPanel,
                onHidePanel: index_onHidePanel,
                onLoadSuccess: function (node, data) {
                    index_addMousedownDiv(this,'value_fixed');
                }

            });

            if(dwbm_jsonData.length > 0){
                $('#value_fixed').combotree('setValue',dwbm_jsonData[0].id);
            }
        },
...
function load_gjcxtj() {
    $.ajax({
        ...
        dataType:'json', 
        success:function (data) {
            zx_gjcx_jsonData = data.data;
            suanfa_loadData(zx_gjcx_jsonData);
        },
        error:function (result) {
...
function load_sf(suanfa_data,sfId) {
    $('#'+sfId).combobox({
        ...
        onSelect:function (node) {//选择算法加载对应的运算符和值
            ...
            var curValueId = "value_"+curIndex;
            for(var i = 0;i < zx_gjcx_jsonData.length;i++){
                if(zx_gjcx_jsonData[i].mc == node.value){
                    var curYSFdata = zx_gjcx_jsonData[i].ysf;
                    //为当前value绑定所需要的bh,bm,parent
                    $("#"+curValueId).attr("data-bh",zx_gjcx_jsonData[i].bh);
                    $("#"+curValueId).attr("data-bm",zx_gjcx_jsonData[i].bm);
                    $("#"+curValueId).attr("data-parent",zx_gjcx_jsonData[i].parent);
                    $("#"+curValueId).attr("data-gyzd",zx_gjcx_jsonData[i].gyzd);
                    $("#ysf_"+curIndex).combobox({
                        valueField:'value',
                        textField:'text',
                        editable:false,
                        data:famaterYSF(curYSFdata)
                    });
                    switch (zx_gjcx_jsonData[i].lx){
                        case 'specialTree':
                            $("#"+curValueId).val("");
                            $("#"+curValueId).removeAttr("data-tip");
                            $("#"+curValueId).addClass("easyui-combotree");
                            $.parser.parse("#value_box_"+curIndex);
                        ...
                        case 'combotree':
                            ...
                            break;
                        case 'input':
                            $("#value_box_"+curIndex).empty();
                            $("#value_box_"+curIndex).append('<input data-tip="input" data-gyzd='+ zx_gjcx_jsonData[i].gyzd +' data-bm='+ zx_gjcx_jsonData[i].bm +' data-parent='+ zx_gjcx_jsonData[i].parent +' data-bh='+ zx_gjcx_jsonData[i].bh +'  style="height:22px;" class="easyui-textbox sf_value" id='+ curValueId +'>')
...
function addNewRow() {
    ...
    if(row_count > 0){
        $('.tiaojian_delet').show();
        $('.tiaojian_delet').css('display','block');
    }
    var html = '';
    html += '<div class="tj_row" id=row_'+row_count+'>';
    html +='<div class="tj_box_sf">';
    html +='<input style="height:22px;" class="easyui-combobox sf" id=sf_'+ row_count +' data-index='+ row_count +'>';
    html +='</div>';
    html +='<div class="tj_box_ysf">';
    html +='<input style="height:22px;" class="easyui-combobox ysf" id=ysf_'+ row_count +'>';
    html +='</div>';
    html +='<div class="tj_box_value" id=value_box_'+ row_count +'>';
    html +='<input style="height:22px;" class="easyui-textbox sf_value" id=value_'+ row_count +'>';
    html +='</div>';
    html +='<div class="tj_box_action" data-index='+ row_count +'>';
    // html +='<div class="tiaojian_btn tiaojian_add" onclick="addNewRow()">增加</div>';
    html +='<div class="tiaojian_btn tiaojian_delet" onclick="delRow(this)">x</div>';
    html +='</div>';
    html += '</div>';
    html += '</div>';
    $("#gjcx_tiaojian").append(html);
    $.parser.parse('#row_'+row_count);
    load_sf(suanfa_data,'sf_'+row_count);
}
//删除条件行
function delRow(obj){
    var rowArr = $(".tj_row");
...
function click_cx(){
    $('#cx_btn').click(function () {
        ...
        var gridOpts = $('#gjcx_datagrid').datagrid('getPager').data('pagination').options;
        var selectCbdw = $('#value_fixed').combotree('getValue');
        var startDate = $('#date_eval_build_gjcx_dept_begin').datebox('getValue');
        var endDate = $('#date_eval_build_gjcx_dept_end').datebox('getValue');
        request.page = gridOpts.pageNumber;
        request.pageSize = gridOpts.pageSize;
        request.parmarter=postArr;
        request.extCbdw =  selectCbdw;
        request.bdw="N";
        request.pcflbm = EVAL_DATA.PCFLBM;
...
function init_gjcxGrid(){
        ...
        columns:[[
            ...
            {field: 'action', title: '操作', width: 90, align: 'center',
                formatter: function (value, row, index) {
                    var r = '<a href="#" onclick="pcWinPage(' + index +',\'#gjcx_datagrid\',1)">查看 </a> ';
                    /*var r = '<a href="#" onclick="lookup_special_case(\'' + row.BMSAH + '\')">查看</a> ';*/
                    var d = '<a href="#" onclick="filter_special_gjcx_case(' + index + ',\'' + row.BMSAH + '\')">筛选</a> ';
                    return r + d;
                }
            }
        ]],
    ...
    $('#gjcx_datagrid').datagrid('getPager').pagination({
        beforePageText: '第',
    ...
    $.ajax({
        ...
        success:function (data) {
            if(data.data == null || data.data.length == 0){
                $("#gjcx_datagrid").datagrid("loadData",{total:0,rows:[]});
            }else{
                $("#gjcx_datagrid").datagrid("loadData",{total:data.total,rows:data.data});
            }
            CloseProgress();
        },
        error:function () {
