...
// 获取评查类容表格中的数据
function load_grid_handle_grade_list() {
    ...
    $('#grid_handle_grade_list').datagrid({
        ...
        columns: [[
            ...
            {
                field: 'pcyj', title: '<span class=\'headPCTitle\' >具体理由</span>', width: 230, rowspan: 2,paddingRight: 20,
                editor: {type: 'textbox', options: {height: 65,paddingRight: 20,multiline: 'true'}},
                styler: function (value, row, index) {
                    return ' position: relative;left: -10px;';
                }
            }
            /*{
                field: 'action',
            ...
            {
                field: 'f1flmc', width: 40, align: 'center',
                formatter: function (value, row, index) {
                    return TwoWordLine(value, '14');  //两字一行，字体大小为14
                },
                styler: function (value, row, index) {
...
function save_grade_score() {
    ...
    for (var i = 0; i < rowDatas.length; i++) {
        ...
        // 评查扣分
        var score = $(".input_ok_checkbox")[i].checked ? '1' : '0';
...
function get_grid_handle_grade_list_auto_item() {
    ...
    for (i = 0; i < rowDatas.length; i++) {
        ...
        //$('#grid_handle_grade_list').datagrid('endEdit', i); //不能结束编辑状态，否则表格会错位
        $('#grid_handle_grade_list').datagrid('beginEdit', i);
        editIndex = i;
...
function grid_handle_grade_list_load_success(data) {
    ...
    $('#grid_handle_grade_list').datagrid('fixRowHeight');
}
...
function grid_handle_grade_list_click_cell(index, field, value) {
    ...
    if (editIndex != index) {
        if ((editIndex == undefined) || ($('#grid_handle_grade_list').datagrid('validateRow', index))) {
            //$('#grid_handle_grade_list').datagrid('endEdit', index); //不能结束编辑状态，否则表格会错位
        ...
        success: function (result) {
            if (result.code == 200) {
                ...
                if (!isNull(data.fz)){
                    tagScoreChange = true;
                    if(data.fz > 0) {
                        $(".input_ok_checkbox")[index].parentNode.classList.remove('img_ok_checkbox');
                        $(".input_ok_checkbox")[index].parentNode.classList.add('img_cancel_checkbox');
                        $(".input_ok_checkbox")[index].checked = true;
                    } else {
                        $(".input_ok_checkbox")[index].parentNode.classList.remove('img_cancel_checkbox');
                        $(".input_ok_checkbox")[index].parentNode.classList.add('img_ok_checkbox');
                        $(".input_ok_checkbox")[index].checked = false;
                    }
                }
...
function update_handle_grade_problems() {
    ...
    for (i = 0; i < rowDatas.length; i++) {
        reduce += $(".input_ok_checkbox")[i].checked ? 1 : 0;
    }

    // 问题总数更新
    //var cnt = $('#lbl_handle_grade_value').text(); //convertToInt(cnt) +
    $('#lbl_handle_grade_value').text(reduce);
