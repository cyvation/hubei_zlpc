...
public class ExportWordUtils {
    ...
    public String exportMillCertificateWord(HttpServletRequest request, HttpServletResponse response, WordVo wordVo) throws IOException {
        ...
        configuration.setDirectoryForTemplateLoading(new File("D:/tomcat/apache-tomcat-9.0.0.M3/webapps/cqe/WEB-INF/classes/static/template/ftlFile"));//模板位置
        Template freemarkerTemplate = configuration.getTemplate("11.ftl");//模板
        Map map=new HashMap();
        File file = null;
        ...
        try {
             ...
             file = createDoc(map,freemarkerTemplate,"D:/tomcat/apache-tomcat-9.0.0.M3/webapps/cqe/WEB-INF/classes/static/template/test.doc");
             fin = new FileInputStream(file);
             response.setCharacterEncoding("utf-8");
             response.setContentType("application/msword");
                 // 设置浏览器以下载的方式处理该文件名
             String fileName = wordVo.getFileName();
             response.setHeader("Content-Disposition", "attachment;filename=".concat(String.valueOf(URLEncoder.encode(test.doc, "UTF-8"))));
